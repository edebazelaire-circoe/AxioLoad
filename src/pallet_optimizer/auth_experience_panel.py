...
_STABILITY = f'<script src="/static/stability_controls.js?v={APP_VERSION}"></script>'.encode()
_LEGACY_ASSETS = (
    ...
    b'<script src="/static/session_controls_guard.js?v=0.22.1"></script>',
)
...
def install_auth_experience_injection() -> None:
    ...
    def template_response(self: Jinja2Templates, *args: Any, **kwargs: Any) -> Any:
        ...
        if b'id="open-settings"' in body or b'id="login-form"' in body:
            for asset in (*_LEGACY_ASSETS, _STYLE, _SCRIPT, _STABILITY):
                body = body.replace(asset, b"")
            body = body.replace(b"</head>", _STYLE + b"</head>")
            body = body.replace(b"</body>", _SCRIPT + _STABILITY + b"</body>")
            response.body = body
