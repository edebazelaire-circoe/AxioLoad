...
from .route_optimization import (
    RouteInputError,
    compare as route_compare_service,
    geocode as route_geocode_service,
...
def create_app(data_dir: str | Path | None = None) -> FastAPI:
    ...
    app = FastAPI(title="AxioLoad", version="0.10.0")
    ...
    def route_compare(payload: dict[str, Any]) -> dict[str, Any]:
        try:
            return route_compare_service(payload)
        except RouteInputError as exc:
            raise HTTPException(422, str(exc)) from exc
        except Exception as exc:
    ...
    def route_optimize(payload: dict[str, Any]) -> dict[str, Any]:
        try:
            return route_optimise(payload)
        except RouteInputError as exc:
            raise HTTPException(422, str(exc)) from exc
        except Exception as exc:
