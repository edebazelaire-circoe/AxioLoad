...
def _draw_header(pdf: canvas.Canvas, width: float, height: float, run: dict[str, Any], solution: dict[str, Any]) -> float:
    ...
    pdf.drawString(110, height - 105, f"Méthode : {solution.get('method_name', 'Méthode historique')}")
    pdf.setFont("Helvetica", 10)
    pdf.drawString(360, height - 105, f"{solution['vehicle_count']} véhicule(s)")
    pdf.drawString(465, height - 105, f"Longueur : {solution['occupied_length_m']:.3f} m")
    pdf.drawString(610, height - 105, f"LDM : {solution['total_linear_meters']:.3f} m.l.")
    return height - 124
