    ...
    with _connect(registry.tenant_path(tenant_id)) as db:
        ...
        for name, declaration in _COLUMNS.items():
            if name not in existing:
                db.execute(f"ALTER TABLE optimization_runs ADD COLUMN {name} {declaration}")
...
def _default_title(request_payload: Mapping[str, Any], optimization_type: str) -> str:
    ...
    if supplied:
        return supplied[:180]
    labels = {
        "loading": "Optimisation de chargement",
        "route": "Optimisation d’itinéraire",
        "total": "Optimisation totale",
    }
    items = request_payload.get("items")
    if optimization_type == "total" and isinstance(request_payload.get("loading"), Mapping):
        items = request_payload["loading"].get("items")
...
def _save_run(
    ...
    with _connect(self.registry.tenant_path(tenant_id)) as db:
        db.execute(
            "UPDATE optimization_runs SET optimization_type='loading', title=? WHERE id=?",
...
def _row_metadata(row: sqlite3.Row) -> dict[str, Any]:
    vehicle_snapshot = []
    try:
        vehicle_snapshot = json.loads(row["vehicle_snapshot_json"] or "[]")
    except (TypeError, ValueError, json.JSONDecodeError):
        vehicle_snapshot = []
    support_intervention = bool(row["admin_touched_at"] or row["created_by_type"] == "super_admin")
    return {
        "optimization_type": row["optimization_type"] or "loading",
        "title": row["title"] or "Optimisation",
        "validation_status": row["validation_status"] or "pending",
        "validated_at": row["validated_at"],
        "validated_by": row["validated_by"],
        "selected_solution": row["selected_solution"],
        "validation_comment": row["validation_comment"] or "",
        "created_by_type": row["created_by_type"] or "user",
        "created_by_id": row["created_by_id"],
        "created_by": row["created_by"],
        "admin_touched_at": row["admin_touched_at"],
        "admin_touched_by": row["admin_touched_by"],
        "support_intervention": support_intervention,
        "support_label": "Intervention du support AxioLoad" if support_intervention else "",
...
def _list_runs(self: TenantRunRepository, tenant_id: str, limit: int = 50) -> list[dict[str, Any]]:
    ...
    with _connect(self.registry.tenant_path(tenant_id)) as db:
        rows = db.execute(
            """SELECT id,status,elapsed_seconds,created_at,result_json,optimization_type,title,
               validation_status,validated_at,validated_by,selected_solution,validation_comment,
               created_by_type,created_by_id,created_by,admin_touched_at,admin_touched_by,vehicle_snapshot_json
    ...
    for row in rows:
        ...
        best = solutions[0] if solutions else result
        vehicle_count = best.get("vehicle_count")
        if vehicle_count is None and row["optimization_type"] == "route":
            vehicle_count = 1
        output.append(
            {
                "id": row["id"],
...
def _get_run(self: TenantRunRepository, tenant_id: str, run_id: str) -> dict[str, Any]:
    ...
    with _connect(self.registry.tenant_path(tenant_id)) as db:
        row = db.execute(
            """SELECT id,status,request_json,result_json,elapsed_seconds,created_at,optimization_type,title,
               validation_status,validated_at,validated_by,selected_solution,validation_comment,
               created_by_type,created_by_id,created_by,admin_touched_at,admin_touched_by,vehicle_snapshot_json
    ...
    return {
        ...
        **metadata,
        "decision": {
            ...
            "decisionAt": metadata["validated_at"],
            "user": metadata["validated_by"],
            "selectedSolution": metadata["selected_solution"],
...
def validate_optimization(
    ...
    if optimization_type not in {"loading", "route", "total"}:
        raise ValueError("Type d’optimisation inconnu")
    title = str(payload.get("title") or "").strip()
    if not title:
        raise ValueError("Le titre de l’optimisation est obligatoire")
    title = title[:180]
    user = str(payload.get("user") or "Utilisateur local").strip()[:180]
    comment = str(payload.get("comment") or "").strip()[:1000]
    selected_raw = payload.get("selected_solution")
    ...
    if run_id:
        with _connect(repository.registry.tenant_path(tenant_id)) as db:
            updated = db.execute(
                """UPDATE optimization_runs SET optimization_type=?,title=?,validation_status='validated',
        ...
        if not result_payload:
            raise ValueError("Le résultat à valider est manquant")
        run_id = str(uuid.uuid4())
        status = str(result_payload.get("status") or "completed")
        elapsed = float(result_payload.get("elapsed_seconds") or 0.0)
        with _connect(repository.registry.tenant_path(tenant_id)) as db:
            db.execute(
                """INSERT INTO optimization_runs(id,status,request_json,result_json,elapsed_seconds,created_at,
